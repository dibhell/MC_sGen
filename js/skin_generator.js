 ...
 * Synthesizes a valid dual-layer 64x64 PNG texture
 */
...
class SkinGenerator {
    ...
    static generateCanvas(profile) {
        ...
        const skinLight = this.shade(skinBase, 1.06);
        ...
        const shirtLight = this.shade(shirtBase, 1.05);
        ...
        const shortLight = this.shade(shortBase, 1.10);
        ...
        // Boki głowy (Right: 0..7, 8..15; Left: 16..23, 8..15; Back: 24..31, 8..15)
        ...
        // Nos (r=5, c=3,4)
        setPixel(8 + 3, 8 + 5, noseColor);
        setPixel(8 + 4, 8 + 5, noseColor);

        // Wąsy i zarost (r=5..7)
        if (hasBeard) {
            setPixel(8 + 1, 8 + 5, beardLight);
            setPixel(8 + 2, 8 + 5, beardColor);
            setPixel(8 + 5, 8 + 5, beardColor);
            setPixel(8 + 6, 8 + 5, beardLight);

            // Uśmiech na r=6
            setPixel(8 + 0, 8 + 6, beardMid);
            setPixel(8 + 1, 8 + 6, beardLight);
            setPixel(8 + 2, 8 + 6, beardColor);
            setPixel(8 + 3, 8 + 6, mouthSmile);
            setPixel(8 + 4, 8 + 6, mouthSmile);
            setPixel(8 + 5, 8 + 6, beardColor);
            setPixel(8 + 6, 8 + 6, beardLight);
            setPixel(8 + 7, 8 + 6, beardMid);

            // Broda w szpic na r=7
            setPixel(8 + 1, 8 + 7, beardMid);
            setPixel(8 + 2, 8 + 7, beardLight);
            setPixel(8 + 3, 8 + 7, beardColor);
            setPixel(8 + 4, 8 + 7, beardColor);
            setPixel(8 + 5, 8 + 7, beardLight);
            setPixel(8 + 6, 8 + 7, beardMid);
        } else {
            setPixel(8 + 3, 8 + 6, mouthSmile);
            setPixel(8 + 4, 8 + 6, mouthSmile);
        }
        ...
        if (profile.hasGlasses) {
            ...
            setPixel(40 + 3, 8 + 4, glassFrame);
            setPixel(40 + 4, 8 + 4, glassFrame);
            setPixel(40 + 7, 8 + 4, glassFrame);

            // Dół oprawek (r=5)
            setPixel(40 + 1, 8 + 5, glassSoft);
            setPixel(40 + 2, 8 + 5, glassSoft);
            setPixel(40 + 5, 8 + 5, glassSoft);
            setPixel(40 + 6, 8 + 5, glassSoft);

            // Zauszniki na bokach
            setPixel(32 + 7, 8 + 4, glassFrame);
            setPixel(32 + 6, 8 + 4, glassFrame);
            setPixel(48 + 0, 8 + 4, glassFrame);
            setPixel(48 + 1, 8 + 4, glassFrame);
        }

        // Wypukła broda 3D
        if (hasBeard) {
            setPixel(40 + 1, 8 + 6, beardColor);
            setPixel(40 + 6, 8 + 6, beardColor);
            setPixel(40 + 2, 8 + 7, beardLight);
            setPixel(40 + 3, 8 + 7, beardColor);
            setPixel(40 + 4, 8 + 7, beardColor);
            setPixel(40 + 5, 8 + 7, beardLight);

            // Od spodu brody
            setPixel(48 + 3, 0 + 0, beardColor);
            setPixel(48 + 4, 0 + 0, beardColor);
        }

        // ----------------------------------------------------
        // 3. TORS (BAZA)
        // ----------------------------------------------------
        // Ramiona góra (20, 16, 8, 4)
        for (let r = 0; r < 4; r++) {
            for (let c = 0; c < 8; c++) {
                setPixel(20 + c, 16 + r, (r === 3 && (c >= 2 && c <= 5)) ? skinShade : shirtLight);
            }
        }
        // Dół krocze (28, 16, 8, 4)
        for (let r = 0; r < 4; r++) {
            for (let c = 0; c < 8; c++) {
        ...
        for (let r = 0; r < 12; r++) {
            for (let c = 0; c < 8; c++) {
                if (r < 8) {
                    setPixel(20 + c, 20 + r, (r + c) % 2 === 0 ? shirtBase : shirtLight);
                } else if (r === 8) {
                    setPixel(20 + c, 20 + r, shirtDark); // rąbek koszulki
                } else {
                    setPixel(20 + c, 20 + r, (r + c) % 2 === 0 ? shortBase : shortLight); // spodenki
                }
            }
        }
        ...
        if (hasLeaves) {
            ...
            for (let [lx, ly] of leaves) {
                setPixel(20 + lx, 20 + ly, (lx + ly) % 2 === 0 ? leafMid : leafDark);
            }
        }

        // Kołnierzyk i sznurek w spodenkach
        setPixel(20 + 2, 20 + 0, shirtCollar);
        setPixel(20 + 3, 20 + 0, skinShade);
        setPixel(20 + 4, 20 + 0, skinShade);
        setPixel(20 + 5, 20 + 0, shirtCollar);

        setPixel(20 + 3, 20 + 9, [240, 240, 240, 255]);
        setPixel(20 + 4, 20 + 9, [240, 240, 240, 255]);
        ...
        for (let faceX of [40, 44, 48, 52]) {
            for (let c = 0; c < 4; c++) setPixel(faceX + c, 36 + 3, shirtBase);
        }
        for (let faceX of [48, 52, 56, 60]) {
            for (let c = 0; c < 4; c++) setPixel(faceX + c, 52 + 3, shirtBase);
        }
        ...
        if (isSlides) {
            ...
            setPixel(20 + 1, 52 + 10, lidlRed);
            setPixel(20 + 2, 52 + 10, lidlYellow);

            // Nakładka 3D paska klapków
            setPixel(4 + 1, 36 + 10, lidlYellow);
            setPixel(4 + 2, 36 + 10, lidlRed);
            setPixel(4 + 1, 52 + 10, lidlRed);
